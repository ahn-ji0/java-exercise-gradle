...
class ParseSeoulHospitalTest {
    ...
    private void assertWhole(SeoulHospital seoulHospital, String id, String address, String district, String type, int er, String name, String subdivision){

        Assertions.assertEquals(id, seoulHospital.getId());

        Assertions.assertEquals(address, seoulHospital.getAddress());

        Assertions.assertEquals(district, seoulHospital.getDistrict());

        Assertions.assertEquals(type, seoulHospital.getType());

        Assertions.assertEquals(er, seoulHospital.getEr());

        Assertions.assertEquals(name, seoulHospital.getName());

        Assertions.assertEquals(subdivision, seoulHospital.getSubdivision());
    ...
    void Parsing(){
        ParseSeoulHospital parseSeoulHospital2 = new ParseSeoulHospital();

        SeoulHospital seoulHospital = parseSeoulHospital2.parsing(this.line1);
